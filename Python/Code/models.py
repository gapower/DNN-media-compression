...
class ModelClass:
    ...
    def train(
        ...
        # Use this to specify when training should auto-stop
        # Useful when using environments such as Colab which disconnect you without warning after 12h-24h
        ...
        history = model.fit(
            ...
            shuffle=True,
            # NOTE In some versions of keras, you are required to update initial_epoch when continuing training
            # This is a bug. Therefore, you may need to change this parameter to the last epoch trained if resuming
            # training.
            initial_epoch=0,
        )
...
"""
VIDEO MODELS - Implemented by G. Power
"""

# 3x3 kernels (MC-model in report)
class GP_3D(ModelClass):
    ...
    def build(self):
        ...
        zpad2 = ZeroPadding3D(padding=(0, 1, 1))(merge2)
        conv3_1 = Conv3D(filters=27, kernel_size=(2, 3, 3), activation="relu")(zpad2)
        conv3_2 = Conv3D(filters=27, kernel_size=(2, 3, 3), activation="relu")(zpad2)
...
# 7x7 kernels (non-MC model in report)
class GP_3D_1(ModelClass):
    ...
    def build(self):
        ...
        zpad1 = ZeroPadding3D(padding=(0, 2, 3))(self.input)
...
# 5x5 kernels (not in report due to poor performance)
class GP_3D_2(ModelClass):
    def __init__(self, dims, precision="float32", **kwargs):
        super().__init__(
            dims, precision, **kwargs
        )  # Is equivalent to super(Attempt1, self).__init__(dims)
    ...
    def build(self):
        ...
        zpad2 = ZeroPadding3D(padding=(0, 0, 0))(merge2)
        conv3_1 = Conv3D(filters=27, kernel_size=(2, 5, 5), activation="relu")(zpad2)
        conv3_2 = Conv3D(filters=27, kernel_size=(2, 5, 5), activation="relu")(zpad2)
        merge3 = concatenate([conv3_1, conv3_2], axis=1)
        ...
        zpad3 = ZeroPadding3D(padding=(0, 4, 4))(skip1)
        conv5_1 = Conv3D(filters=27, kernel_size=(2, 5, 5), activation="relu")(zpad3)
        conv5_2 = Conv3D(filters=27, kernel_size=(2, 5, 5), activation="relu")(zpad3)
        merge5 = concatenate([conv5_1, conv5_2], axis=1)
        ...
        zpad4 = ZeroPadding3D(padding=(0, 5, 4))(skip2)
        conv6_1 = Conv3D(filters=9, kernel_size=(2, 5, 5), activation="relu")(zpad4)
...
# 7x7 + less downsampling (not in report due to poor performance)
class GP_3D_3(ModelClass):
    def __init__(self, dims, precision="float32", **kwargs):
        super().__init__(
            dims, precision, **kwargs
        )  # Is equivalent to super(Attempt1, self).__init__(dims)
    ...
    def build(self):
        ...
        zpad1 = ZeroPadding3D(padding=(0, 3, 3))(self.input)
        conv1_1 = Conv3D(filters=3, kernel_size=(2, 7, 7), activation="relu")(zpad1)
        conv1_2 = Conv3D(filters=3, kernel_size=(2, 7, 7), activation="relu")(zpad1)
        merge1 = concatenate([conv1_1, conv1_2], axis=1)
        ...
        conv3_2 = Conv3D(filters=9, kernel_size=(2, 2, 2), strides=(1, 2, 2), activation="relu")(merge2)
        merge3 = concatenate([conv3_1, conv3_2], axis=1)
        ...
        conv4_1x = Conv3D(filters=27, kernel_size=(2, 7, 7), activation="relu")(zpad3x)
        conv4_2x = Conv3D(filters=27, kernel_size=(2, 7, 7), activation="relu")(zpad3x)
        merge4x = concatenate([conv4_1x, conv4_2x], axis=1)

        zpad4x = ZeroPadding3D(padding=(0, 3, 3))(merge4x)
        conv4_1 = Conv3D(filters=81, kernel_size=(2, 7, 7), activation="relu")(zpad4x)
        conv4_2 = Conv3D(filters=81, kernel_size=(2, 7, 7), activation="relu")(zpad4x)
        merge4 = concatenate([conv4_1, conv4_2], axis=1)

        skip1 = concatenate([merge4, merge3x], axis=4)
        ...
        skip2 = concatenate([up2, merge2], axis=4)
...
"""
IMAGE MODELS - Implemented by R. Trew
"""
...
"""
VIDEO MODELS - Implemented by R. Trew
"""
